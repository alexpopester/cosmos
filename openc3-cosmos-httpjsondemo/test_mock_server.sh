...
while getopts "n:N:p:H:P:t:k:i:h" opt; do
  case $opt in
...
while true; do
  ...
  HTTP_CODE=\$(curl -s -o /dev/null -w '%{http_code}' \\
    -X POST "http://${COSMOS_HOST}:${COSMOS_SERVER_PORT}/${TARGET_NAME}/${INBOUND_PACKET}" \\
    -H 'Content-Type: application/json' \\
    -d "{\\"temperature\\":\$TEMP,\\"pressure\\":\$PRES,\\"label\\":\\"\$LABEL\\"}" \\
    2>/dev/null || echo "000")
